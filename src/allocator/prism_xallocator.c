...
static int is_arena_enabled(void) { (void)arena_enabled; return 1; }
static int is_arena_log_enabled(void) { const char *env = getenv("LEUKO_ARENA_LOG"); return (env && env[0] == '1'); }

void x_allocator_begin_parse(void) {
    if (!is_arena_enabled()) return;
    if (is_arena_log_enabled()) fprintf(stderr, "[arena] begin_parse\n");
    pthread_mutex_lock(&arena_lock);
    if (arena_head) { arena_free(arena_head); arena_head = NULL; }
...
typedef struct mp_hdr { uint64_t magic; size_t size; } mp_hdr_t;
...
static void *arena_alloc_wrapper(size_t size) {
    ...
    pthread_mutex_lock(&arena_lock);
    if (!arena_head) {
        arena_head = arena_new(ARENA_CHUNK_DEFAULT);
        if (!arena_head) { pthread_mutex_unlock(&arena_lock); return NULL; }
    }
    if (size > ARENA_SMALL_LIMIT) { pthread_mutex_unlock(&arena_lock); return malloc(size); }
    void *p = arena_alloc(arena_head, total);
    if (!p) { pthread_mutex_unlock(&arena_lock); return NULL; }
    mp_hdr_t *h = (mp_hdr_t *) p; h->magic = ARENA_MAGIC; h->size = size;
...
void *prism_alloc_impl(size_t size) {
    if (!is_arena_enabled()) { __sync_fetch_and_add(&sys_malloc_calls, 1); return malloc(size); }
...
void *prism_calloc_impl(size_t nmemb, size_t size) {
    ...
    if (!is_arena_enabled()) { __sync_fetch_and_add(&sys_calloc_calls, 1); return calloc(nmemb, size); }
...
void *prism_realloc_impl(void *ptr, size_t size) {
    if (!ptr) return prism_alloc_impl(size);
    if (!is_arena_enabled()) { __sync_fetch_and_add(&sys_realloc_calls, 1); return realloc(ptr, size); }
    if (ptr_in_arena(ptr)) {
        mp_hdr_t *oh = (mp_hdr_t *)((char *)ptr - sizeof(mp_hdr_t));
        size_t old_size = 0; if (oh->magic == ARENA_MAGIC) old_size = oh->size;
...
void prism_free_impl(void *ptr) {
    if (!ptr) return;
    if (!is_arena_enabled()) { __sync_fetch_and_add(&sys_free_calls, 1); free(ptr); return; }
    if (ptr_in_arena(ptr)) return; /* no-op */
