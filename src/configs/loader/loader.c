...
static bool apply_config_multi(config_t *cfg, yaml_document_t **docs, size_t doc_count, char **err)
{
...
bool config_apply_file(config_t *cfg, const char *path, char **err)
{
    ...
    /* Do not allocate docs yet; iterate raws directly for AllCops extraction */
    ...
    /* Allocate docs array only when invoking apply */
    if (raw_count == 0)
    {
        return true;
    }
    ...
    bool ok = apply_config_multi(cfg, docs, doc_count, err);
