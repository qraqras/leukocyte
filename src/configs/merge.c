...
yaml_document_t *yaml_merge_rule_mapping_multi(yaml_document_t **docs, size_t doc_count, const char *full_name, const char *category_name)
{
    ...
    for (size_t di = 0; di < doc_count; di++)
    {
        ...
        /* allcops mapping */
        yaml_node_t *allcops = yaml_get_mapping_node(doc, r, ALL_COPS);
        if (allcops)
        {
            mm_merge_from_mapping(&root, doc, allcops);
        }

        /* category then rule */
        yaml_node_t *category = NULL;
        if (category_name)
        {
            yaml_node_t *cat = yaml_get_mapping_node(doc, r, category_name);
            if (cat)
            {
                category = cat;
                mm_merge_from_mapping(&root, doc, category);
                yaml_node_t *rule_in_cat = yaml_get_mapping_node(doc, cat, full_name ? strrchr(full_name, '/') ? strrchr(full_name, '/') + 1 : full_name : NULL);
                if (rule_in_cat)
                {
                    mm_merge_from_mapping(&root, doc, rule_in_cat);
                }
            }
        }

        /* top-level rule mapping */
        yaml_node_t *rule_node = yaml_get_mapping_node(doc, r, full_name);
        if (rule_node)
        {
            mm_merge_from_mapping(&root, doc, rule_node);
        }
    }
